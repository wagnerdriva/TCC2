                    ...
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            ...
                            if save_crop:
                                bbox_w = output[2] - output[0]
                                bbox_h = output[3] - output[1]
                                if (bbox_h >= min_height and bbox_w >= min_width and conf.item() > min_conf):
                                    txt_file_name = txt_file_name if (isinstance(path, list) and len(path) > 1) else ''
                                    img = save_one_box(bboxes, imc, file=save_dir / 'crops' / txt_file_name / names[c] / f'{id}' / f'{conf:.2f}-{p.stem}.jpg', BGR=True, save=False)
                                    ...
                                    future = producer.send(os.getenv("KAFKA_PRODUCER_TOPIC"), data)
